...
void Indicator::install() {
    ...
    m_installProcess.start("bash /opt/click.ubuntu.com/indicator-torch.nfsprodriver/current/indicator/install.sh");
}

void Indicator::uninstall() {
    //TODO don't hardcode this
    m_uninstallProcess.start("bash /opt/click.ubuntu.com/indicator-torch.nfsprodriver/current/indicator/uninstall.sh");
}
...
bool Indicator::checkInstalled() {
    QFileInfo session("/home/phablet/.config/upstart/nfsprodriver-indicator-torch.conf");
    QFileInfo indicator("/home/phablet/.local/share/unity/indicators/com.nfsprodriver.indicator.torch");
